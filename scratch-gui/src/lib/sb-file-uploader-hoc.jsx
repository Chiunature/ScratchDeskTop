    ...
    class SBFileUploaderComponent extends React.Component {
        ...
        async handleOpenFileFromOS(filePath) {
            ...
            try {
                ...
                const fileContent = await window.myAPI.readFiles(
                    filePath,
                    "",
                    {}
                );
                ...
                if (!fileContent) {
                    throw new Error(
                        "读取文件失败: fileContent 为 null 或 undefined"
                    );
                }

                if (!fileContent.buffer) {
                    throw new Error("文件内容无效: 缺少 buffer 属性");
                }
                ...
                await this.props.vm
                    ...
                    .catch((error) => {
                        log.warn("[SBFileUploader] VM 加载项目失败", error);
                        const errorMsg =
                            error?.message ||
                            error?.toString() ||
                            String(error);
                ...
                log.warn(
                    "[SBFileUploader] handleOpenFileFromOS 执行失败",
                    error
                );
                alert(
                    this.props.intl.formatMessage(messages.loadError) +
        ...
        onload() {
            if (this.fileReader) {
                ...
                this.props.vm
                    ...
                    .catch((error) => {
                        log.warn(
                            "[SBFileUploader] VM 加载项目失败 (onload)",
                            error
                        );
                        const errorMsg =
                            error?.message ||
                ...
                console.warn(
                    "[SBFileUploader] onload 被调用但 fileReader 为 null"
                );
            }
        }
