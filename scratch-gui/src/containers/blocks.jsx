...
class Blocks extends React.Component {
        ...
        requestIdleCallback(
            () => {
                ...
                toolboxWorkspace.registerButtonCallback(
                    "MAKE_A_VARIABLE",
                    varListButtonCallback("variable")
                );
                toolboxWorkspace.registerButtonCallback(
                    "MAKE_A_LIST",
    ...
    workspaceToCode(type) {
        if (type === "move" || type === "change" || type === "delete") {
            const code = this.ScratchBlocks[
                this.props.generatorName
            ].workspaceToCode(this.workspace);
    ...
    onWorkspaceUpdate(data) {
        ...
        if (!sessionStorage.getItem("setDefaultStartBlock"))
...
Blocks.defaultOptions = {
    ...
    grid: {
        spacing: 40, // 网格间距`
        length: 2, // 网格线长度 点状背景才有效
        colour: "rgb(226, 243, 255)", // 网格线颜色
        gridType: "grid",
    },
    colours: {
        workspace: "#F9F9F9",
...
const mapDispatchToProps = (dispatch) => ({
    ...
    onSetMatchMsgBlock: (matchMsgBlock) =>
        dispatch(setMatchMsgTaskBlock(matchMsgBlock)),
});
