...
Blockly.cake["matrix_lamp"] = function (block) {
  ...
  let color = Blockly.cake.valueToCode(
    block,
    "COLOR",
  ...
  let code = `char BMP${no}[] = {${lp}};\nmatrix_lamp(BMP${no}, ${
    Blockly.cake.toStr(newColor) ? newColor : '"' + newColor + '"'
  });\n`;
  return code;
};
...
Blockly.cake["matrix_lamp_set"] = function (block) {
  let brightness = Blockly.cake.valueToCode(
    block,
    "brightness",
  ...
  // TODO: Assemble cake into code variable.
  let code = `matrix_set_lamp(${
    Blockly.cake.toStr(brightness) ? brightness : '"' + brightness + '"'
  });\n`;
  return code;
};
...
Blockly.cake["matrix_lamp_single"] = function (block) {
  ...
  // TODO: Assemble cake into code variable.
  let code = `matrix_lamp_single(${
    Blockly.cake.toStr(x) ? x : '"' + x + '"'
  }, ${Blockly.cake.toStr(y) ? y : '"' + y + '"'}, ${
    Blockly.cake.toStr(brightness) ? brightness : '"' + brightness + '"'
  });\n`;
  return code;
};
...
Blockly.cake["matrix_lamp_text"] = function (block) {
  let text = Blockly.cake.valueToCode(
    block,
    "matrix_text",
    Blockly.cake.ORDER_NONE
  );
  ...
  if (match && match.length > 0 && text.indexOf("matrix") === -1)
    text = match[0].toUpperCase();
  // let code = `char Text[] = {${hex.join(",")}};\nmatrix_text_lamp(Text);\n`;
  let code = `matrix_text_lamp(${
    Blockly.cake.toStr(text) ? text : '"' + text + '"'
  });\n`;
  return code;
};
...
Blockly.cake["matrix_lamp_setRGB"] = function (block) {
  ...
  // TODO: Assemble cake into code variable.
  let code = `matrix_setRGB_lamp(${
    Blockly.cake.toStr(order) ? order : '"' + order + '"'
  }, ${color.replace(/\'/g, '"')});\n`;
  return code;
};
...
Blockly.cake["matrix_lamp_setSaturation"] = function (block) {
  let saturation = Blockly.cake.valueToCode(
    block,
    "saturation",
    Blockly.cake.ORDER_ATOMIC
  );
  ...
  if (
    typeof saturation === "string" &&
    (saturation.indexOf("(") !== -1 || saturation.indexOf("_") !== -1)
  ) {
...
Blockly.cake["matrix_lamp_setColorRGB"] = function (block) {
  ...
  // TODO: Assemble cake into code variable.
  let code = `matrix_lamp_setColorRGB(${
    Blockly.cake.toStr(r) ? r : '"' + r + '"'
  }, ${Blockly.cake.toStr(g) ? g : '"' + g + '"'}, ${
    Blockly.cake.toStr(b) ? b : '"' + b + '"'
  });\n`;
  return code;
};
