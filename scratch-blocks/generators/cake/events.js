...
Blockly.cake["event_when"] = function (block) {
  var argument =
    Blockly.cake.valueToCode(block, "CONDITION", Blockly.cake.ORDER_NONE) ||
    "False";
  var branch = Blockly.cake.statementToCode(block, "SUBSTACK");
  branch = Blockly.cake.addLoopTrap(branch, block.id);
  var code =
    "while(true)\n{\nwhile (strcmp(" +
    `${Blockly.cake.toStr(argument) ? argument : '"' + argument + '"'}` +
    ', "TRUE") != 0){vTaskDelay(50);}\n';
  code += branch;
...
Blockly.cake["event_whenmicrobitbuttonpressed"] = function (block) {
  ...
  Blockly.cake.loops_["event_whenmicrobitbegin" + key + i] =
    "if button_" +
    key +
    ".is_pressed() {\n" +
    Blockly.cake.INDENT +
  ...
  if (!nextBlock) {
    ...
    for (var x in Blockly.cake.variables_) {
      variablesName.push(
        Blockly.cake.variables_[x].slice(
          0,
          Blockly.cake.variables_[x].indexOf("=") - 1
        )
      );
    }
...
Blockly.cake["event_whenmicrobitpinbeingtouched"] = function (block) {
  ...
  Blockly.cake.loops_["event_whenmicrobitpinbeingtouched" + pin + i] =
    "if pin" +
    pin +
    ".is_pressed() {\n" +
    Blockly.cake.INDENT +
  ...
  if (!nextBlock) {
    ...
    if (variablesName.length !== 0) {
      code +=
        Blockly.cake.INDENT + "global " + variablesName.join(", ") + "\n}";
    }
...
Blockly.cake["event_whenmicrobitgesture"] = function (block) {
  ...
  Blockly.cake.loops_["event_whenmicrobitgesture" + sta + i] =
...
Blockly.cake["event_checkcolor"] = function (block) {
  ...
  let code = `int checkColor = ${cr.replace(
    /\'/g,
    ""
...
Blockly.cake["event_whengreaterthan"] = function (block) {
  ...
  let value = Blockly.cake.valueToCode(
    block,
    "VALUE",
    Blockly.cake.ORDER_ATOMIC
  );
...
Blockly.cake["event_broadcast_menu"] = function (block) {
  // TODO: Assemble cake into code variable.
  let varName = Blockly.cake.variableDB_.getName(
