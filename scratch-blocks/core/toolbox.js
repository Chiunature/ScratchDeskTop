 ...
 * This is the sum of the width of the flyout (250) and the category menu (135).
 * @type {number}
 */
Blockly.Toolbox.prototype.width = 385;
...
Blockly.Toolbox.prototype.createTrigger = function () {
  var trigger = goog.dom.createDom(
    goog.dom.TagName.DIV,
    "blocklyToolboxTrigger"
  );
  var triggerIcon = goog.dom.createDom(
  ...
  triggerIcon.setAttribute(
    "src",
    Blockly.mainWorkspace.options.pathToMedia + "hide.svg"
  );
  triggerIcon.style.right = "-462px";
...
/**
 * 修改 隐藏时this.triggerIcon_ 的图标和位置
 */
Blockly.Toolbox.prototype.unfoldTrigger = function () {
  this.triggerIcon_.style.right = "-161px";
  this.triggerIcon_.setAttribute(
    "src",
    Blockly.mainWorkspace.options.pathToMedia + "show.svg"
  );
};

/**
 * 修改 展开时this.triggerIcon_ 的图标和位置
 */
Blockly.Toolbox.prototype.foldTrigger = function () {
  this.triggerIcon_.style.right = "-462px";
  this.triggerIcon_.setAttribute(
    "src",
    Blockly.mainWorkspace.options.pathToMedia + "hide.svg"
  );
...
Blockly.Toolbox.prototype.populate_ = function (newTree) {
  ...
  this.setSelectedItemForPopulate(this.categoryMenu_.categories_[0], false);
};
...
Blockly.Toolbox.prototype.setSelectedItemForPopulate = function (
  item,
  opt_shouldScroll
) {
  if (typeof opt_shouldScroll === "undefined") {
...
Blockly.Toolbox.Category.prototype.createDom = function () {
  ...
  if (this.iconURI_) {
    ...
    // 支持图片背景，如果有iconURI则使用图片，否则使用颜色作为后备
    // 如果路径不是以 http:// 或 https:// 或 data: 开头，尝试添加 basePath
    var iconPath = this.iconURI_;
    // if (!iconPath.match(/^(https?:\/\/|data:|\/)/)) {
    //   // 尝试从 workspace 获取 basePath
    //   try {
    //     var workspace = this.parent_.parent_.workspace_;
    //     if (workspace && workspace.options) {
    //       var basePath = workspace.options.basePath || "./";
    //       // 确保 basePath 以 / 结尾（如果不是 ./）
