 ...
 * @const
 */
Blockly.FieldPiano.SHADOW_OPACITY = .2;
 ...
 * @const
 */
Blockly.FieldPiano.NUM_WHITE_KEYS = 8;
 ...
 * @const
 */
Blockly.FieldPiano.MAX_NOTE = 36;
 ...
 * @const
 */
Blockly.FieldPiano.ANIMATION_FRACTION = 0.2;
...
Blockly.FieldPiano.prototype.showEditor_ = function() {
  ...
  Blockly.FieldPiano.superClass_.showEditor_.call(this, this.useTouchInteraction_);
  ...
  this.fieldEditorWidth_ = Blockly.FieldPiano.NUM_WHITE_KEYS * Blockly.FieldPiano.WHITE_KEY_WIDTH +
    Blockly.FieldPiano.EDGE_PADDING;
  this.fieldEditorHeight_ = Blockly.FieldPiano.TOP_MENU_HEIGHT +
  ...
  this.keySVGs_ = [];
  this.addPianoOctave_(-this.fieldEditorWidth_ + Blockly.FieldPiano.EDGE_PADDING,
  ...
  var highCX = lowCX + (Blockly.FieldPiano.WHITE_KEY_WIDTH *
    (Blockly.FieldPiano.NUM_WHITE_KEYS - 1));
  ...
  this.octaveUpButton = this.addOctaveButton_(
      (this.fieldEditorWidth_ + Blockly.FieldPiano.INSET * 2) -
      Blockly.FieldPiano.OCTAVE_BUTTON_SIZE, false, svg);
 ...
 * @private
 */
Blockly.FieldPiano.prototype.addPianoOctave_ = function(x, whiteKeyGroup, blackKeyGroup, keySVGarray) {
  ...
  for (var i = 0; i < Blockly.FieldPiano.KEY_INFO.length; i++) {
    ...
    if (Blockly.FieldPiano.KEY_INFO[i].isBlack) {
    ...
    if (keySVGarray) {
      ...
      keySVG.setAttribute('data-pitch', Blockly.FieldPiano.KEY_INFO[i].pitch);
      keySVG.setAttribute('data-name', Blockly.FieldPiano.KEY_INFO[i].name);
      keySVG.setAttribute('data-isBlack', Blockly.FieldPiano.KEY_INFO[i].isBlack);
...
Blockly.FieldPiano.prototype.getPianoKeyPath_ = function(x, y, width, height) {
  return  'M' + x + ' ' + y + ' ' +
    'L' + x + ' ' + (y + height -  Blockly.FieldPiano.KEY_RADIUS) + ' ' +
    'Q' + x + ' ' + (y + height) + ' ' +
    (x + Blockly.FieldPiano.KEY_RADIUS) + ' ' + (y + height) + ' ' +
    'L' + (x + width - Blockly.FieldPiano.KEY_RADIUS) + ' ' + (y + height) + ' ' +
    'Q' + (x + width) + ' ' + (y + height) + ' ' +
    (x + width) + ' ' + (y + height - Blockly.FieldPiano.KEY_RADIUS) + ' ' +
    'L' + (x + width) + ' ' + y + ' ' +
    'L' + x +  ' ' + y;
};
...
Blockly.FieldPiano.prototype.addOctaveButton_ = function(x, flipped, svg) {
  ...
  arrow.setAttributeNS(
      ...
      'xlink:href',
      Blockly.mainWorkspace.options.pathToMedia + Blockly.FieldPiano.ARROW_SVG_PATH
  );
  Blockly.utils.createSvgElement('line',
      {
        'stroke': this.sourceBlock_.getColourTertiary(),
  ...
  if (flipped) {
    var translateX = -1 * Blockly.FieldPiano.OCTAVE_BUTTON_SIZE + (Blockly.FieldPiano.INSET * 2);
...
Blockly.FieldPiano.prototype.addCKeyLabel_ = function(x, svg) {
  return Blockly.utils.createSvgElement('text',
      {
        'x': x,
        'y': Blockly.FieldPiano.TOP_MENU_HEIGHT + Blockly.FieldPiano.WHITE_KEY_HEIGHT -
 ...
 * @private
 */
Blockly.FieldPiano.prototype.setCKeyLabelsVisible_ = function(visible) {
  if (visible) {
 ...
 * @private
 */
Blockly.FieldPiano.prototype.fadeSvgToOpacity_ = function(svg, opacity) {
 ...
 * @private
 */
Blockly.FieldPiano.prototype.changeOctaveBy_ = function(octaves) {
  this.displayedOctave_ += octaves;
  if (this.displayedOctave_ < 0) {
    this.displayedOctave_ = 0;
    return;
  }
  var maxOctave = Math.floor(Blockly.FieldPiano.MAX_NOTE / 12) - 1;
  ...
  var newNote = Number(this.noteNum_) + (octaves * 12);
 ...
 * @private
 */
Blockly.FieldPiano.prototype.stepOctaveAnimation_ = function() {
  ...
  this.animationPos_ += (this.animationTarget_ - this.animationPos_) *
    Blockly.FieldPiano.ANIMATION_FRACTION;
...
Blockly.FieldPiano.prototype.updateSelection_ = function () {
  ...
  var noteNum = Number(this.noteNum_);
  ...
  if (this.keySVGs_[index]) {
    this.keySVGs_[index].setAttribute('fill', Blockly.FieldPiano.KEY_SELECTED_COLOR);
    // Update the note name text
    var noteName =  Blockly.FieldPiano.KEY_INFO[index].name;
...
Blockly.FieldPiano.prototype.classValidator = function(text) {
  ...
  if (n > Blockly.FieldPiano.MAX_NOTE) {
    n = Blockly.FieldPiano.MAX_NOTE;
  }
  return String(n);
};
