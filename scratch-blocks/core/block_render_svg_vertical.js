 ...
 * 注意：路径值需要与 NOTCH_WIDTH 和 NOTCH_HEIGHT 匹配
 * 当 GRID_UNIT=4 时：NOTCH_WIDTH=32px, NOTCH_HEIGHT=8px
 * 路径值按比例缩放以适应 GRID_UNIT 的变化
 * @const
 */
Blockly.BlockSvg.NOTCH_PATH_LEFT = (function () {
  var scale = Blockly.BlockSvg.GRID_UNIT / 4; // 缩放比例（默认4，现在是5）
  // 原始值（基于 GRID_UNIT=4）
  var c1 = Math.round(2 * scale);
  var c2 = Math.round(3 * scale);
  var c3 = Math.round(4 * scale);
  var h = Math.round(12 * scale);
  var v = Blockly.BlockSvg.GRID_UNIT; // 垂直值等于 GRID_UNIT（NOTCH_HEIGHT = 2 * GRID_UNIT，但路径中只用一半）
  return (
    "c " +
    c1 +
    ",0 " +
 ...
 * @const
 */
Blockly.BlockSvg.NOTCH_PATH_RIGHT = (function () {
  var scale = Blockly.BlockSvg.GRID_UNIT / 4; // 缩放比例（默认4，现在是5）
  // 原始值（基于 GRID_UNIT=4）
  var c1 = Math.round(2 * scale);
  var c2 = Math.round(3 * scale);
