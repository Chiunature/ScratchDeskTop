...
Blockly.Grid = function (pattern, options) {
  ...
  /**
   * The type of grid background: 'grid' for full grid lines, 'dot' for dot pattern.
   * @type {string}
   * @private
   */
  this.gridType_ = options["gridType"] || "grid"; // 默认为栅格背景
};
...
Blockly.Grid.prototype.update = function (scale) {
  ...
  // 改为栅格背景：线条贯穿整个网格区域
  var half = (this.spacing_ / 2) * scale;
  if (this.gridType_ === "grid") {
    // 水平线：从 x=0 到 x=safeSpacing，y 位置在中间
    this.setLineAttributes_(this.line1_, scale, 0, safeSpacing, half, half);
    // 垂直线：从 y=0 到 y=safeSpacing，x 位置在中间
    this.setLineAttributes_(this.line2_, scale, half, half, 0, safeSpacing);
  } else {
    // 点状背景：使用短线条
    var halfOriginal = Math.floor(this.spacing_ / 2) + 0.5;
    var start = (halfOriginal - this.length_ / 2) * scale;
    var end = (halfOriginal + this.length_ / 2) * scale;
